...
from backend.core.optimizers.compressor import PromptCompressor, UtilityIndex  # noqa: E402
...
from backend.domain.workload import (  # noqa: E402
    BASELINE_MODEL, CACHEABLE_STEPS, CASCADE_NEXT, CONFIDENCE, ChunkPool, LOOP_CALLS_PER_MIN,
    LOOP_IN, LOOP_KILL_DELAY_MIN, LOOP_OUT, PROMPT_BLOAT_FACTOR, QUALITY, QUERY_TEMPLATES,
    SUCCESS_QUALITY_THRESHOLD, Step, TENANTS, WORKFLOWS, query_text,
)
...
# ---- planted incidents (batch run only) --------------------------------------
        ...
        for plan in self.demand:
            ...
            for step in plan.steps:
                ...
                prompt = ("system instructions\n\nyou are a helpful assistant.\n\n"
                          + query_text(plan.template_id, plan.paraphrase, plan.workflow))
                ...
                # --- semantic cache -----------------------------------------
                qtext = query_text(plan.template_id, plan.paraphrase, plan.workflow) + f" {step.step}"
